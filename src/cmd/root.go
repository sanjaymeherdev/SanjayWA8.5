...
import (
        ...
        "whatsapp-bot/infrastructure/chatstorage"
...
var (
        ...
        aiUsecase         domainAI.IAIUsecase
)

// rootCmd represents the base command when called without any subcommands
var rootCmd = &cobra.Command{
        Short: "Send free whatsapp API",
        Long: `This application is from clone https://whatsapp-bot/src, 
...
func initEnvConfig() {
        ...
        // Groq AI settings
        if envGroqAPIKey := viper.GetString("groq_api_key"); envGroqAPIKey != "" {
                config.GroqAPIKey = envGroqAPIKey
        }
        if envGroqModel := viper.GetString("groq_model"); envGroqModel != "" {
                config.GroqModel = envGroqModel
        }
        if envGroqMaxTokens := viper.GetInt("groq_max_tokens"); envGroqMaxTokens > 0 {
                config.GroqMaxTokens = envGroqMaxTokens
        }
}
